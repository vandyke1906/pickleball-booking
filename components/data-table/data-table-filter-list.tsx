...
import { getDefaultFilterOperator, getFilterOperators } from "@/lib/data-table.v3"
...
export function DataTableFilterList<TData>({
  ...
  const columns = React.useMemo(() => {
    return table.getAllColumns().filter(
      (column) =>
        column.columnDef.enableColumnFilter !== false && // explicit false excludes it
        column.columnDef.id !== "actions" && // safety net
        column.getCanFilter(), // respect TanStack logic
    )
  }, [table])
        ...
        <div className="min-w-[72px] text-center">
          {index === 0 ? (
            ...
            >
              ...
              >
                {dataTableFilterV3Config.joinOperators.map((joinOperator) => (
    ...
    case "multiSelect": {
      ...
      const selectedValues = multiple
        ...
        : typeof filter.value === "string" && filter.value
          ? filter.value
          : undefined
      const options = dynamicFilterOptions?.[filter.id] ?? columnMeta?.options ?? []
      ...
      return (
        <Faceted
          open={showValueSelector && !isLoading}
          onOpenChange={setShowValueSelector}
          value={selectedValues}
          onValueChange={(value) => {
            const currentValue = multiple ? value : ((value as string[])[0] ?? "")
            onFilterUpdate(filter.filterId, { value: currentValue })
          }}
          multiple={multiple}
        >
          <FacetedTrigger asChild>
            <Button
              ...
              disabled={isLoading}
            >
              {/* <FacetedBadgeList options={options} placeholder={displayText} maxBadges={multiple ? 2 : 1} overflowSuffix=" more" /> */}
              <FacetedBadgeList
                options={options}
          ...
          <FacetedContent id={inputListboxId} className="w-[min(360px,90vw)] max-h-[320px]">
            <FacetedInput
            ...
            <FacetedList>
              {isLoading ? (
                ...
                <FacetedEmpty>No options available</FacetedEmpty>
              ) : (
                <>
                  <FacetedEmpty>No matching options found</FacetedEmpty>
                  <FacetedGroup>
                    {options.map((option) => (
                      <FacetedItem
                        key={option.value}
