...
function FacetedBadgeList(props: FacetedBadgeListProps) {
  ...
  return (
    <div {...badgeListProps} className={cn("flex flex-wrap items-center gap-1", className)}>
      {values.length > max ? (
        <Badge variant="primary" size="xs" className={cn("rounded-sm px-1 font-normal", badgeClassName)}>
          {values.length} selected
        </Badge>
      ) : (
        values.map(value => (
          <Badge key={value} variant="primary" size="xs" className={cn("rounded-sm px-1 font-normal", badgeClassName)}>
...
function FacetedItem(props: FacetedItemProps) {
  ...
  return (
    <CommandItem aria-selected={isSelected} data-selected={isSelected} className={cn("gap-2", className)} onSelect={() => onItemSelect(value)} {...itemProps}>
      <span
        className={cn(
