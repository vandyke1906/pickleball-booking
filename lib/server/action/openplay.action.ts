...
import {
  manager,
  QUEUE_KEYS,
} from "@/lib/server/services/queue-manager.service";
import {
  TCurrentGame,
  TQueueGroup,
  TQueuePlayer,
...
export async function initializeLineup(
  tx: TPrismaTransaction,
  openPlayId: string,
) {
  ...
  const playersGroup: Array<
    (typeof players)[number] & { openPlayGroupId: string }
  > = [];
  ...
  for (const player of playersGroup) {
    jobs.push(
      manager.addJob(QUEUE_KEYS.LINEUP_PLAYER, player.openPlayGroupId, player),
    );
  }
...
export async function canJoinQueue(
  ...
  const elapsedMinutes = Math.floor(
    (currentTime.getTime() - player.startAt.getTime()) / 60000,
  );
...
function getNextAvailableCourt(
  courtMap: Record<string, { date: Date; isFirst: boolean }>,
) {
...
export async function scheduleGroup(group: any[], tx?: TPrismaTransaction) {
  ...
  const now = new Date();
  const baseStartAt = isFirst
    ? new Date(availableAt)
    : new Date(availableAt.getTime() + buffer);
  const startAt =
    baseStartAt > now ? baseStartAt : new Date(now.getTime() + buffer);
  ...
  for (const player of group) {
    ...
    const computedEndAt = new Date(
      (queue.scheduledAt?.getTime() ?? startAt.getTime()) +
        player.totalPlayTime * 60000,
    );
...
export async function getNewOpenPlaySchedules(
  ...
  const courtsWithGames = activeOpenPlay.courts.map((court) => {
    ...
    const currentTime = times.find(
      (t) =>
        t <= now.getTime() &&
        groupsByTime[t].some((q) => !q.endedAt || q.endedAt > now),
    );
    if (currentTime) {
      currentGame = {
        courtId: court.id,
        ...
        estimatedEndTime: new Date(
          currentTime + activeOpenPlay.transitionMinutes * 60000,
        ),
        isPreparing:
          now.getTime() <
          currentTime + activeOpenPlay.preparationSeconds * 1000,
      };
    }
    ...
    if (nextTime) {
      nextGame = {
        ...
        estimatedEndTime: new Date(
          nextTime + activeOpenPlay.transitionMinutes * 60000,
        ),
        position: 1,
      };
    }
  ...
  const futureGrouped = groupByCourtAndTime(
    activeOpenPlay.queues.filter(
      (q: any) => q.scheduledAt && q.scheduledAt > now,
    ),
  );
  ...
  Object.entries(currentGrouped).forEach(([key, players]) => {
    ...
    if (court) {
      currentGames.push({
        ...
        isPreparing:
          now.getTime() <
          scheduledAt.getTime() + activeOpenPlay.preparationSeconds * 1000,
      });
    }
  });
  ...
  if (options?.onGroupDone) {
    await Promise.all(
      completedGames.map((game) => options.onGroupDone?.(game)),
    );
  }
