...
export async function initializeLineup(tx: TPrismaTransaction, openPlayId: string) {
  ...
  // Group players by matching openPlayGroupId
  const groupedPlayers = new Map<string, typeof players>()

  for (const player of players) {
    const groupId = skillGroupMap.get(player.skill)

    if (!groupId) continue

    if (!groupedPlayers.has(groupId)) {
      groupedPlayers.set(groupId, [])
    }

    groupedPlayers.get(groupId)!.push(player)
  }

  // Alternate players between groups
  const groupIds = [...groupedPlayers.keys()]
  const playersGroup: Array<(typeof players)[number] & { openPlayGroupId: string }> = []
  ...
  while (added) {
    ...
    for (const groupId of groupIds) {
      const queue = groupedPlayers.get(groupId)!
...
export async function submitLineup(
  ...
  // Add job to manager
  await manager.addJob(QUEUE_KEYS.LINEUP_PLAYER, "lineup-player", {
    ...player,
    openPlayGroupId: opGroup.id,
  })
}
...
async function getCourtAvailability(openPlayId: string, tx?: TPrismaTransaction) {
  ...
  const courtMap: Record<string, { date: Date; isFirst: boolean; name: string }> = {}

  // initialize with OpenPlay startTime
  for (const c of openPlay.courts) {
    courtMap[c.id] = { date: new Date(openPlay.startTime), isFirst: true, name: c.name }
  }
  ...
  for (const q of openPlay.queues) {
    if (q.assignedCourtId && q.endedAt) {
      ...
      if (!current || q.endedAt > current.date) {
        courtMap[q.assignedCourtId] = {
          date: q.endedAt,
          isFirst: false,
          name: q.assignedCourt?.name || "",
        }
      }
    }
  }
...
function getNextAvailableCourt(
  courtMap: Record<string, { date: Date; isFirst: boolean; name: string }>,
) {
  const sorted = Object.entries(courtMap).sort(
    (a: any, b: any) => a[1].date.getTime() - b[1].date.getTime(),
  )
  const [courtId, { date, isFirst, name }] = sorted[0]
  return { courtId, availableAt: date, isFirst, name }
}
...
export async function scheduleGroup(group: any[], tx?: TPrismaTransaction) {
  ...
  const {
    ...
    isFirst,
    name: courtName,
  } = await getNextCourt(openPlayId, db)
  ...
  const { transitionMinutes, preparationSeconds, announcementMinutesBeforeTransition } = openPlay
...
export async function getNewOpenPlaySchedules(
  ...
  const courtsWithGames = activeOpenPlay.courts.map((court: any) => {
    const courtQueues = activeOpenPlay.queues.filter((q: any) => q.assignedCourtId === court.id)
    ...
    if (currentTime) {
      currentGame = {
        ...
        players: groupsByTime[currentTime].map(toQueuePlayer).sort((a, b) =>
    ...
    if (nextTime) {
      nextGame = {
        ...
        courtId: court.id,
        courtName: court.name,
        players: groupsByTime[nextTime].map(toQueuePlayer).sort((a, b) =>
          a.playerName.localeCompare(b.playerName, undefined, {
            sensitivity: "base",
          }),
        ),
  ...
  const unscheduled = activeOpenPlay.queues.filter((q: any) => !q.scheduledAt && !q.endedAt)
  ...
  activeOpenPlay.groups.forEach((g: any) => {
    const hasPlayers = activeOpenPlay.queues.some((q: any) => q.openPlayGroupId === g.id)
    if (!hasPlayers) {
      waitingGroups.push({
        groupId: g.id,
        skills: g.skills,
